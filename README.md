...
The current client/server pack is Friends MC 1.3.0. The client `.mrpack` is
stored with Git LFS; after this one-time import, AutoModpack applies future pack
updates incrementally. BlueMap is intentionally server-only and the browser map
is available only to approved portal users.
